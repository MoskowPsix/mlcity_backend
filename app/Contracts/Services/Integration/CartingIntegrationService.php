...
use App\Constants\CartingSourceConstants;
...
class CartingIntegrationService
{
    public function createOrGetEvent(CartingCreateEventRequest $request): Event
    {
        $existing = Event::query()
            ->where('source_name', CartingSourceConstants::SOURCE_NAME)
        ...
        DB::beginTransaction();
        try {
            $userId = (int) config('services.carting.user_id', 1);
            ...
            $typeId = $this->resolveCartingEventTypeId();
            ...
            $event = Event::create([
                'name' => $request->name,
                'sponsor' => CartingSourceConstants::SPONSOR,
                ...
                'source_name' => CartingSourceConstants::SOURCE_NAME,
            ...
            // Правило для контента с картинга: сразу «Опубликовано», без модерации
            ...
            DB::rollBack();
            Log::error('CartingIntegrationService::createOrGetEvent failed', [
    ...
    public function updateEvent(Event $event, CartingCreateEventRequest $request): Event
    {
        DB::beginTransaction();
        try {
            $location = $this->resolveLocation((float) $request->latitude, (float) $request->longitude);
            ...
            $event->types()->sync([$this->resolveCartingEventTypeId()]);
            ...
            // Карта «Места» читает coords у Sight, не у Place — обновляем связанный картинг-sight
            $this->syncLinkedCartingSightCoords(
                $linkedSightId,
                $request,
                $location,
            );
            ...
            DB::rollBack();
            Log::error('CartingIntegrationService::updateEvent failed', [
                'source_id' => $request->sourceId,
                'error' => $e->getMessage(),
            ]);
            throw $e;
    ...
    public function createOrGetSight(CartingCreateSightRequest $request): Sight
    {
        ...
        try {
            $userId = (int) config('services.carting.user_id', 1);
            $location = $this->resolveLocation((float) $request->latitude, (float) $request->longitude);
            $typeId = $this->resolveCartingSightTypeId();
            $publishedStatusId = $this->publishedStatusId();
            ...
            DB::rollBack();
            Log::error('CartingIntegrationService::createOrGetSight failed', [
                'source_id' => $request->sourceId,
                'error' => $e->getMessage(),
            ]);
            throw $e;
    ...
    public function updateSight(Sight $sight, CartingCreateSightRequest $request): Sight
    {
        DB::beginTransaction();
        try {
            $location = $this->resolveLocation((float) $request->latitude, (float) $request->longitude);
            ...
            $sight->types()->sync([$this->resolveCartingSightTypeId()]);
            ...
            DB::rollBack();
            Log::error('CartingIntegrationService::updateSight failed', [
                'source_id' => $request->sourceId,
                'error' => $e->getMessage(),
            ]);
            throw $e;
    ...
    /**
     * Пишет URL картинок картинга в sight_files (local=0, без копирования файла).
     *
    ...
    /**
     * Пишет URL картинок картинга в event_files (local=0, без копирования файла).
     *
     * @param  list<string>|null  $images
     */
    ...
    /**
     * Синхронизация цен по платным классам картинга.
     *
    ...
    private function syncExternalImages($model, ?array $images, string $context): void
    {
        ...
        if (!$typeId) {
            Log::warning("CartingIntegrationService::syncExternalImages: FileType image not found", [
                'context' => $context,
                'id' => $model->id,
            ]);
    ...
    private function resolveSightForEvent(CartingCreateEventRequest $request, Location $location): ?Sight
    {
    ...
    private function syncLinkedCartingSightCoords(
        ?int $sightId,
        CartingCreateEventRequest $request,
        Location $location,
    ): void {
        if (!$sightId) {
            return;
    ...
    private function resolveLocation(float $latitude, float $longitude): Location
    {
        ...
        if (!$location) {
            throw new Exception('Не найден город (location) для координат картинга');
        }
    ...
    private function resolveCartingEventTypeId(): int
    {
        $type = EventType::query()->where('name', CartingSourceConstants::TYPE_NAME)->first();
        if ($type) {
            return (int) $type->id;
        }

        $type = EventType::create([
            'name' => CartingSourceConstants::TYPE_NAME,
    ...
    private function resolveCartingSightTypeId(): int
    {
        $type = SightType::query()->where('name', CartingSourceConstants::TYPE_NAME)->first();
        if ($type) {
            return (int) $type->id;
        }
