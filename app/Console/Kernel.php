...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->command('type')->daily()->skip(function () {
            return true;
        })->description('Прверка оповещения всех пользователей');
        $schedule->command('inоtegration:min-cult all')->daily()->skip(function () {
            return true;
        })->description('Интеграция с сервисом мин-культ');;
        $schedule->command('integration:vld all')->daily()->skip(function () {
            return true;
        })->description('Интеграция с сервисом Влада');
        ...
        $schedule->command('search:index')->daily()->skip(function () {
            return true;
        })->description('Проиндекчировать записи для elasticsearch');
        $schedule->command('double-type:del')->daily()->skip(function () {
            return true;
        })->description('Убрать дублирующиеся типы у сообществ и удалить сообщества без мероприятий');
        $schedule->command('backup_db')->daily()->skip(function () {
            return true;
