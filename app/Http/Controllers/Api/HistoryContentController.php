...
use App\Contracts\Services\EventHistoryContentService;
use App\Contracts\Services\SightHistoryContentService;
...
class HistoryContentController extends Controller
{
    ...
    private function checkStatuses(){
        ...
        if(request("type") == "Event") {
            ...
            return Sight::where('id', request('id'))->whereHas('statuses', function($q) use ($status_id){
                $q->where('status_id', $status_id)->where('last', true);
            })->exists();
        }
    }
