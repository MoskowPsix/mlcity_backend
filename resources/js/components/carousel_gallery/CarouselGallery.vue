<template lang="">
    ...
    >
        <div class="w-[100%] mx-auto rounded-lg">
            <div
                v-if="stateDrop && wrightState"
                class="absolute flex border left-1/2 top-[9rem] w-48 h-48 z-50 bg-slate-300/70 dark:bg-gray-900/70 dark:border-gray-700/50 border-gray-200/90 text-gray-800/70 dark:text-gray-400/70 p-3 rounded-lg"
            >
                <svg
            ...
            >
                <Slide
                    v-for="slide in files"
                    :key="slide"
                    class="rounded-lg"
                >
                    <div
                        class="absolute w-[100%] h-[100%] blur-lg bg-gray-50/10 z-20 rounded-lg"
            ...
            <div class="flex flex-row min-h-[7rem] min-w-[7rem]">
                <div
                    v-if="wrightState"
                    class="dark:text-gray-400/70 dark:bg-gray-700/70 hover:dark:text-gray-400 hover:dark:bg-gray-700 bg-slate-400/30 border-slate-400/40 text-gray-600 hover:bg-slate-400/60 hover:text-gray-700/90 w-[6rem] h-[6rem] m-3 rounded-lg p-0.2"
                    @click="chooseFiles()"
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                ...
                >
                    <Slide
                        ...
                        :key="slide"
                        :class="{
                            'bg-green-500/50': false,
                            'bg-red-500/50': false,
                            'bg-blue-500/50': false,
                        }"
                    >
                        <div
                            class="flex flex-col max-h-[7rem] max-w-[7rem]"
                            @click="slideTo(index)"
                        >
                            <img
                                :src="slide.link"
                            ...
                            <button
                                v-if="wrightState"
                                class="absolute dark:text-red-200 text-red-600 bg-red-200/70 dark:bg-red-600/70 p-1 rounded-lg"
                                @click="emitDelImg(slide)"
                            >
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    fill="none"
