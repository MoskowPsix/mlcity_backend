<template lang="">
    ...
    <table class="border border-gray-300 dark:border-gray-700 rounded-lg w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            <tr @click.prevent="clickContent(content)" class="transition bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 cursor-pointer active:scale-95" v-for="content in contents">
                <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                    {{content.id}}
                </th>
                <td class="px-6 py-4">
                    <div class="flex justify-between ">
                        <svg v-if="content.history_contentable_type === 'App\\Models\\Event'" aria-hidden="true" class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white enable-background:new 0 0 24 24" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <svg v-if="content.history_contentable_type === 'App\\Models\\Sight'" aria-hidden="true" class="flex-shrink-0 w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" fill="currentColor" viewBox="0 0 500 600" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <svg v-if="content.cult_id" viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" aria-hidden="true" class="flex-shrink-0 w-6 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white">
                ...
                <td class="px-6 py-4">
                    <h1 class="dark:text-gray-200 text-base">{{content.user.id}} / {{content.user.name}}</h1>
                        <p class="dark:text-gray-400 text-ml">{{content.user.email}}</p>
                </td>
                <td class="px-6 py-4">
                    {{content.date_start}}
                </td>
