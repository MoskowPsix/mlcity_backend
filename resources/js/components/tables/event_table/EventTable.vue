<template lang="">
    ...
    <table class="border border-gray-300 dark:border-gray-700 rounded-lg w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" v-for="event in events">
                ...
                <td class="px-6 py-4">
                    <div class="flex justify-between ">
                        ...
                        <svg v-if="event.cult_id" viewBox="0 0 128 128" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" aria-hidden="true" class="flex-shrink-0 w-6 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white">
                ...
                <td class="px-6 py-4">
                    <h1 class="dark:text-gray-200 text-base">{{event.author.id}} / {{event.author.name}}</h1>
                        <p class="dark:text-gray-400 text-ml">{{event.author.email}}</p>
                </td>
                <td class="px-6 py-4">
                    {{event.date_start}}
                </td>
