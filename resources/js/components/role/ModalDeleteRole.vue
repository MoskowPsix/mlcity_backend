...
//const store = useUsersStore().closeModal();
 export default defineComponent({
   ...
   methods: {
    closeModal() {
      this.store_role.closeDeleteRole()
    },
    DelRole() {
      this.store_role.deleteRole();
    },
   }  
  })
</script>
		...
		<div
			...
			<div class="modal-content py-6 text-left px-6 ">
				...
				<div class="flex justify-end pt-2">
					...
					<button @click="DelRole()" class="focus:outline-none px-4 bg-red-500 p-3 ml-3 rounded-lg text-grey-300 hover:bg-red-400">Удалить</button>
				</div>
			</div>
		</div>
	</div>
