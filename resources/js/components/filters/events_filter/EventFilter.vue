<template lang="">
    <div>
        ...
        <div class="mb-[0.125rem] block min-h-[1.5rem] pl-7  border rounded-lg dark:bg-gray-800 dark:border-gray-700 border-gray-400/50">
            ...
            <VueDatePicker v-model="eventDate" range model-type="yyyy-MM-dd HH:mm:ss" :class="themeState ? 'w-full h-full mt-1 dp_theme_dark' : 'w-full h-full mt-1 dp_theme_light'" placeholder="Дата и время события" />
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    data() {
        ...
        eventDate:[
            this.getEventDate().split('~')[0].slice(0,19).replace("T", ' '),
            this.getEventDate().split('~')[1].slice(0,19).replace("T", ' ')
        ],
